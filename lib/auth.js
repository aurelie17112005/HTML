...
function decodeBase64Hash(value) {
  ...
  if (!text) return '';
  try {
    return Buffer.from(text, 'base64url').toString('utf8').trim();
  } catch (_) {
    return '';
  }
}

function isValidPasswordHashFormat(value) {
  const parts = String(value || '').split('$');
  if (parts.length !== 6 || parts[0] !== 'scrypt') return false;
  const [N, r, p] = parts.slice(1, 4).map(Number);
  return Number.isInteger(N) && N > 1 && Number.isInteger(r) && r > 0 && Number.isInteger(p) && p > 0
    && Boolean(parts[4]) && Boolean(parts[5]);
}

function passwordHashForUser(user) {
  // Le format Base64URL est recommandé sur Render : il évite toute altération
  // possible des signes "$" présents dans le hash scrypt standard.
  const encoded = process.env[`${user.passwordHashEnv}_B64`];
  const fromBase64 = decodeBase64Hash(encoded);
  if (fromBase64) return { value: fromBase64, source: 'base64' };

  return {
    value: stripWrappingQuotes(process.env[user.passwordHashEnv]),
    source: 'plain',
  };
}
...
function configuredHumanUsers() {
  return HUMAN_USERS.map(user => {
    const password = passwordHashForUser(user);
    return {
      ...user,
      passwordHash: password.value,
      passwordHashSource: password.source,
      passwordHashValid: isValidPasswordHashFormat(password.value),
    };
  });
}
...
function configureAuth(app) {
  ...
  const authConfiguration = configuredHumanUsers().map(user => ({
    id: user.id,
    displayName: user.displayName,
    configured: Boolean(user.passwordHash),
    validHash: Boolean(user.passwordHashValid),
    source: user.passwordHashSource,
  }));
  for (const item of authConfiguration) {
    console.log(`[auth] ${item.displayName}: ${item.configured ? (item.validHash ? `mot de passe configuré (${item.source})` : 'hash présent mais invalide') : 'mot de passe absent'}`);
  }
  console.log(`[auth] environnement=${isRender ? 'render' : (isProduction ? 'production' : 'development')} cookieSecure=${cookieOptions.secure} sameSite=${cookieOptions.sameSite}`);
