...
function miraklIncidentThreadExclusionEnabled(provider) {
  // Conforama remonte parfois des dossiers de type incident dans la messagerie Mirakl.
  // Dans l'écran "Réclamations", on les exclut par défaut pour éviter de mélanger
  // dossiers incident/logistique et vraies conversations client à traiter.
  if (String(process.env.MIRAKL_KEEP_INCIDENT_THREADS_IN_RECLAMATIONS || '').match(/^(1|true|yes|on)$/i)) return false;
  const raw = cleanText(process.env.MIRAKL_EXCLUDE_INCIDENT_THREADS_PROVIDERS || 'conforama').toLowerCase();
  if (!raw || /^(0|false|no|none|off)$/i.test(raw)) return false;
  ...
  return wanted.has(code) || wanted.has(label);
}

function miraklThreadIncidentSignature(thread = {}) {
  const entities = Array.isArray(thread.entities) ? thread.entities : [];
  const entityTypes = entities.flatMap(e => [
    e?.type, e?.entity_type, e?.entityType, e?.name, e?.code, e?.value,
    e?.entity?.type, e?.entity?.entity_type, e?.entity?.entityType,
  ]);
  const values = [
    thread.type, thread.thread_type, thread.threadType, thread.kind,
    thread.context, thread.context_type, thread.contextType,
    thread.object_type, thread.objectType, thread.resource_type, thread.resourceType,
    thread.entity_type, thread.entityType,
    thread.topic?.type, thread.topic?.code, thread.topic?.value, thread.topic?.name, thread.topic?.label,
    thread.reason?.type, thread.reason?.code, thread.reason?.value,
    thread.category?.type, thread.category?.code, thread.category?.value,
    thread.source, thread.origin,
    ...entityTypes,
  ];
  return foldStatusText(values.map(scalarValue).filter(Boolean).join(' '));
}

function isMiraklIncidentThreadForReclamations(provider, thread = {}) {
  if (!miraklIncidentThreadExclusionEnabled(provider)) return false;

  const signature = miraklThreadIncidentSignature(thread);
  if (!signature) return false;

  // On ne se base volontairement pas sur le texte libre du client : un client peut écrire
  // "j'ai un incident" dans une vraie réclamation. On filtre seulement les marqueurs
  // structurels Mirakl/Conforama : type, topic, reason, category, entity type...
  if (/\b(order[_ -]?incident|incident[_ -]?order|order[_ -]?issue|issue[_ -]?order|incident|litige|dispute|case)\b/.test(signature)) {
    return true;
  }
  return false;
}

function filterMiraklThreadsForReclamations(provider, threads = []) {
  if (!Array.isArray(threads) || !threads.length || !miraklIncidentThreadExclusionEnabled(provider)) return threads;
  let excluded = 0;
  const kept = threads.filter(thread => {
    const drop = isMiraklIncidentThreadForReclamations(provider, thread);
    if (drop) excluded += 1;
    return !drop;
  });
  if (excluded) {
    console.log(`[mirakl/${provider.code}] ${excluded} incident(s) filtré(s) hors de l'écran Réclamations`);
  }
  return kept;
}
const mirakl = {
  ...
  async fetchClaims(provider) {
    const threads = filterMiraklThreadsForReclamations(provider, await this.fetchAllThreads(provider));
...
function isProductInformationClaim(claim = {}) {
  const ctx = claim._ctx || {};
  const signature = foldStatusText([
    claim.subject,
    claim.subjectCode,
    claim.reason,
    claim.reasonLabel,
    ...
    ctx.topic,
    ctx.motif,
  ].map(scalarValue).filter(Boolean).join(' '))
    .replace(/[^a-z0-9]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  ...
  // Filtrage volontairement ciblé : on exclut les catégories/motifs structurels
  // de type "information produit", mais on garde les vraies réclamations produit
  // comme produit non conforme, produit endommagé, pièce manquante, etc.
  const hasProductInfoTopic = /\b(product information|information produit|informations produit|information sur le produit|infos produit|info produit|question produit|question sur le produit|renseignement produit|renseignements produit|demande information produit|demande informations produit|demande d information produit|demande d informations produit|product question|product enquiry|product inquiry)\b/.test(signature);
  if (!hasProductInfoTopic) return false;

  const realClaimTerms = /\b(non recu|pas recu|jamais recu|retard|livraison|colis|casse|cassee|endommage|abime|defectueux|panne|sav|garantie|non conforme|mauvais produit|piece manquante|accessoire manquant|retour|remboursement|facture|annulation)\b/.test(signature);
  return !realClaimTerms;
}
...
function productInfoExclusionDefaultEnabled() {
  if (parseBoolFlag(process.env.RECLAMATIONS_KEEP_PRODUCT_INFO_THREADS, false)) return false;
  return parseBoolFlag(process.env.RECLAMATIONS_EXCLUDE_PRODUCT_INFO_THREADS, true);
}
function threadsRequestOptions(req) {
  ...
  const excludeProductInfo = parseBoolFlag(
    req.query.excludeProductInfo ?? req.query.excludeProductInfoThreads ?? process.env.RECLAMATIONS_EXCLUDE_PRODUCT_INFO_THREADS,
    productInfoExclusionDefaultEnabled()
  ) && !parseBoolFlag(req.query.keepProductInfo ?? process.env.RECLAMATIONS_KEEP_PRODUCT_INFO_THREADS, false);
  return {
    onlyUnanswered,
    maxAgeDays,
    excludeProductInfo,
...
function threadsCacheKey(options) {
  return stableCacheKey({
    ...
    excludeProductInfo: options.excludeProductInfo ? 1 : 0,
  });
}
  ...
  await mapLimit(providers, options.concurrency, async (p, idx) => {
    ...
    try {
      ...
      for (const rawClaim of (Array.isArray(fetched) ? fetched : [])) {
        ...
        if (options.excludeProductInfo && isProductInformationClaim(claim)) {
          productInfoExcluded += 1;
          continue;
        }
      ...
      const payload = {
        ...
        ...(productInfoExcluded ? { productInfoExcluded } : {}),
        ok: true,
      };
      providerChunks[idx] = payload;
      if (onProvider) onProvider(payload);
      console.log(`[${p.type}/${p.code || 'octopia'}] ${kept.length}/${Array.isArray(fetched) ? fetched.length : 0} réclamation(s) à répondre, fenêtre=${providerMaxAgeDays || 'illimitée'}j${productInfoExcluded ? `, infos produit filtrées=${productInfoExcluded}` : ''}`);
    } catch (e) {
      completed += 1;
      const payload = {
        provider: info,
